...
// Update the shortcode registration function
function register_trust_badges_shortcodes() {
    // Register existing shortcodes
    // add_shortcode('trust_badges_footer', 'trust_badges_footer_shortcode'); // Commented out footer shortcode
    add_shortcode('trust_badges_checkout', 'trust_badges_checkout_shortcode');
    add_shortcode('trust_badges_product', 'trust_badges_product_shortcode');
    
    // Register dynamic shortcodes for custom groups
    global $wpdb;
    $table_name = $wpdb->prefix . 'converswp_trust_badges';
    
    // Get all custom badge groups
    $custom_groups = $wpdb->get_results(
        "SELECT id, group_id, settings 
        FROM {$table_name} 
    ...
    if ($custom_groups) {
        foreach ($custom_groups as $group) {
            $shortcode_name = 'trust_badges_' . $group->group_id;
            add_shortcode($shortcode_name, function($atts) use ($group) {
                return trust_badges_custom_shortcode($atts, $group);
            });
        }
    }
}
add_action('init', 'register_trust_badges_shortcodes');

/**
 * Shortcode handler for custom badge groups
 */
function trust_badges_custom_shortcode($atts, $group) {
    ...
    $output = '<div class="trust-badges-custom trust-badges-' . esc_attr($group->group_id) . '">';
    
    // Add header if enabled
    if ($settings['showHeader']) {
        $header_style = sprintf(
            'color: %s; font-size: %spx; text-align: %s;',
            esc_attr($settings['textColor']),
            esc_attr($settings['fontSize']),
            esc_attr($settings['alignment'])
        );
        $output .= sprintf('<div class="trust-badges-header" style="%s">%s</div>', 
            $header_style,
            esc_html($settings['headerText'])
        );
    }
    
    // Add badges container
    $badges_style = sprintf(
        'justify-content: %s; %s',
        $settings['badgeAlignment'] === 'left' ? 'flex-start' : ($settings['badgeAlignment'] === 'right' ? 'flex-end' : 'center'),
        $settings['customMargin'] ? sprintf(
    ...
    $output .= sprintf('<div class="trust-badges-container" style="%s" data-animation="%s">', 
        $badges_style,
        esc_attr($settings['animation'])
    );
    
    // Add individual badges
    if (!empty($settings['selectedBadges'])) {
        foreach ($settings['selectedBadges'] as $badge_id) {
            $badge_class = sprintf(
                'trust-badge %s %s',
                $settings['badgeStyle'],
                $settings['badgeStyle'] === 'mono' || $settings['badgeStyle'] === 'mono-card' ? 'mono' : ''
            );
            
            $badge_style = sprintf(
                'background-image: url(%s);%s',
                esc_url(plugins_url("assets/images/badges/{$badge_id}.svg", __FILE__)),
                ($settings['badgeStyle'] === 'mono' || $settings['badgeStyle'] === 'mono-card') ? 
                    sprintf('background-color: %s;', esc_attr($settings['badgeColor'])) : ''
            );
            
            $output .= sprintf(
                '<div class="%s" style="%s" data-desktop-size="%s" data-mobile-size="%s"></div>',
                esc_attr($badge_class),
                $badge_style,
                esc_attr($settings['badgeSizeDesktop']),
                esc_attr($settings['badgeSizeMobile'])
            );
        }
    }
...
// Update the styles function to include custom badge groups
function trust_badges_footer_styles() {
    ?>
    <style>
        /* Common styles for all badge groups */
        .trust-badges-custom {
            width: 100%;
            margin: 0 auto;
        }
        ...
        @media (min-width: 768px) {
            .trust-badge[data-desktop-size="extra-small"] { width: 2rem; height: 2rem; }
            .trust-badge[data-desktop-size="small"] { width: 3rem; height: 3rem; }
            .trust-badge[data-desktop-size="medium"] { width: 4rem; height: 4rem; }
            .trust-badge[data-desktop-size="large"] { width: 5rem; height: 5rem; }
        }
        
        /* Mobile sizes */
        @media (max-width: 767px) {
            .trust-badge[data-mobile-size="extra-small"] { width: 1.5rem; height: 1.5rem; }
            .trust-badge[data-mobile-size="small"] { width: 2rem; height: 2rem; }
            .trust-badge[data-mobile-size="medium"] { width: 2.5rem; height: 2.5rem; }
            .trust-badge[data-mobile-size="large"] { width: 3rem; height: 3rem; }
        }
        
        /* Animations */
        .trust-badges-container[data-animation="fade"] .trust-badge {
            animation: trustBadgesFade 0.4s ease-in-out;
        }
        
        .trust-badges-container[data-animation="slide"] .trust-badge {
            animation: trustBadgesSlide 0.4s ease-in-out;
        }
        
        .trust-badges-container[data-animation="scale"] .trust-badge {
            animation: trustBadgesScale 0.4s ease-in-out;
        }
        
        .trust-badges-container[data-animation="bounce"] .trust-badge {
            animation: trustBadgesBounce 0.4s ease-out;
        }
        
        @keyframes trustBadgesFade {
            from { opacity: 0; }
            to { opacity: 1; }
        }
        
        @keyframes trustBadgesSlide {
            from { transform: translateX(-100px); opacity: 0; }
            to { transform: translateX(0); opacity: 1; }
        }
        
        @keyframes trustBadgesScale {
            from { transform: scale(0); opacity: 0; }
            to { transform: scale(1); opacity: 1; }
        }
        
        @keyframes trustBadgesBounce {
...
add_action('wp_head', 'trust_badges_footer_styles');

function trust_badges_enqueue_scripts() {
    wp_enqueue_script(
        'trust-badges',
        plugins_url('assets/js/trust-badges.js', __FILE__),
        array(),
        '1.0.0',
        true
    );
}
add_action('wp_enqueue_scripts', 'trust_badges_enqueue_scripts');

// Add this function to handle saving custom group settings
function save_custom_badge_group($group) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'converswp_trust_badges';
...
// Add this function to handle shortcode cache
function flush_trust_badges_shortcodes() {
    wp_cache_delete('trust_badges_shortcodes', 'trust-badges');
}
add_action('save_post', 'flush_trust_badges_shortcodes');

// Comment out the footer shortcode handler
/*
function trust_badges_footer_shortcode($atts) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'converswp_trust_badges';
    
    // Get the footer group
    $footer_group = $wpdb->get_row(
        $wpdb->prepare(
            "SELECT * FROM {$table_name} WHERE group_id = %s AND is_active = 1",
            'footer'
        )
    );
    
    // Return empty if footer group not found or not active
    if (!$footer_group) {
        return '';
    }

    // Decode settings
    $settings = json_decode($footer_group->settings, true);
    if (!$settings) {
        return '';
    }
    
    // Start building the HTML
    $output = '<div class="trust-badges-footer">';
